<template lang="">
  <section @focusout="handleFocus" @mousedown="checkValid" @mouseup="checkValid" id="taskForm" class="pb-1">
    ...
    <div class="card-body p-1">
      ...
      <section class="">
        <div class="text-start px-2 fs-6 fw-bold title-row">
          ...
          <section @keydown="handleTitleInputKD" @keyup="handleTitleInputKU" id="titleInput" contenteditable=true class="task-input">       
          </section>
        </div>
        <button
      ...
      <section class="text-start py-3">
        ...
        <div @keydown="handleDetailInputKD" @keyup="handleDetailInputKU" id="detailInput" contenteditable=true class="task-input">
          
        </div>
      </section>

      <section v-if="tagsInput[0]" class="text-start ps-3">
        <span v-for="tag of tagsInput" class="badge rounded-pill border me-1">{{tag.name}} <button @click="removeTag(tag.name, tag.id)" type="button" style="font-size: .75em" class="btn-close" aria-label="Close"></button></span>
      </section>
